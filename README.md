...
- [ ] What problem does the context API help solve?
  Allows you to pass state only to props that need it/removes prop drilling
- [ ] In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
  actions - function/action that the component will do before renduring
  reducer - holds you state and runs all of your actions
  store - where all of the state is held and passes it down to necessary components when connected
- [ ] What is the difference between Application state and Component state? When would be a good time to use one over the other?
  Application State - State that effects the whole application
  Component State - State that only affects the respective component
- [ ] Describe `redux-thunk`, what does it allow us to do? How does it change our `action-creators`?
  allows you to make asyncrounous function calls
- [ ] What is your favorite state management system you've learned and this sprint? Please explain why!
  Redux has grown on me because of the ease of setting up your store and provider. Passing state/actions to the necessary components is quite simple
